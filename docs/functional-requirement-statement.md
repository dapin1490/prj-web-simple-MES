...
- 진척률 산출 로직: 실생산 수량 컬럼이 없으므로 `SEQ_NO` 기반 대체 지표를 사용하여 진행률($P$)을 실시간 계산함.
    
    $$P = \frac{seq_{current}}{seq_{total}} \times 100,\ \ seq_{total}=\max(SEQ\_NO\ \text{for}\ wo\_id)$$
    
    (`seq_total <= 0`인 경우 `P = 0`, 결과는 0~100 범위로 제한)
...
- 최종 실적 집계: 생산 완료 시 `SEQ_NO` 기반 진척률(수량 대체 지표)과 품질 결과를 요약하여 기록함.
